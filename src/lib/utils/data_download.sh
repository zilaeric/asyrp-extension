...
BASE_DIR=$2
    ...
    if [ $DATASET == "celeba_hq" ]; then
        ...
        DATASET_FOLDER="./src/data/celeba_hq"
    ...
    for DATASET_TYPE in "train" "test" "val"; do
        python src/lib/utils/prepare_lmdb_data.py --out $DATASET_FOLDER/LMDB_$DATASET_TYPE --size $TARGET_SIZE $DATASET_FOLDER/raw_images/$DATASET_TYPE
    done
